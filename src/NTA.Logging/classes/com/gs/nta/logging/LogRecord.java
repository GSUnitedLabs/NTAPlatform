 ...
 *  Oct 31, 2021  Sean Carrick         Updated the LogRecord class to implement
 *                                     the GSLogRecord interface for use with
 *                                     ServiceLoader in order to remove the
 *                                     chances of cyclic dependencies, so that
 *                                     com.gs.nta.utils can be refactored back
 *                                     to its own module.
 * *****************************************************************************
 */
 ...
 *         // initialize the Logger instance via ServiceLoader
 * 
 *         ServiceLoader<GSLogRecord> loader = ServiceLoader.load(GSLogRecord.class);
 *         record.setSourceClassName(getClass().getCanonicalName());
 *         record.setSourceMethodName("MyClass [Constructor]"); 
 *         record.setParameters(new Object[] {application});
 *         // The rest of the setters can be called before:
 *         logger.enter(record); 
 * 
 *         // the rest of the class' initialization.
 *         
 *         // If the constructor called other methods, the setSourceMethodName()
 *         // setter should be called again. Other record properties could be set.
 *         logger.exit(record); 
 *     }
 *
 *     public void doSomething(String name, int id, float pay) { 
 *         // Let's assume that name == Joe, id == 5, and pay == 5.5 
 *         record.setParameters(new Object[] {name, id, pay}); 
 ...
 *         + " the product is %2f"); 
 *         record.setParameters(new Object[] {pay, id, calc});
 *         logger.debug(record);
 *
 ...
 * @since 1.0
 */
public class LogRecord implements GSLogRecord {
    ...
    /**
     * Sets the source class name for this `LogRecord`.
     * 
     * @param sourceClassName the name of the class from which this `LogRecord`
     * is being generated
     */
    @Override
